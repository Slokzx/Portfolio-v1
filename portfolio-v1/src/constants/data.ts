export const data = {
  hero: {
    intro: "Hello, my name is,",
    name: "Slok Shah",
    tagline: "I build things for the web.",
    bio: "I’m a Senior Front End engineer focused on accessible, human-centered experiences. Right now I’m building modern products and design systems for teams that care about craft.",
    downloadButton: "Download CV",
    fileName: "Slok Shah Resume",
    downloadLink: "/SlokShahResume.pdf",
  },
  summary: {
    title: "Over the years,",
    description: [
      "I've built products for companies and businesses around the globe ranging from marketing websites to complex solutions and enterprise apps with focus on fast, elegant and accessible user experiences.",

      "I modernized legacy interfaces into a unified React design system collaborating with designers to deliver pixel-perfect and WCAG-compliant UIs. I also developed interactive geospatial visualization tools with React, Leaflet, and MapLibre to display millions of property records in real time. Mentoring a small team, I improved delivery speed by 30% through agile workflows and structured reviews.",

      "At Apple, I built internal React + Redux applications for manufacturing operations, automating manual workflows and improving efficiency by 40%. I established a shared React component library adopted by multiple teams, ensuring design and code consistency. My work on performance optimization, modular architecture, and accessibility set new standards for UI reliability and developer experience across projects.",
    ],
  },
  companies: [
    {
      timeline: {
        startYear: "2021",
        endYear: "PRESENT",
      },
      companyName: "Lightbox",
      title: "Front End Engineer",
      description:
        "Led a frontend team to modernize the UI with a unified React design system, cutting feature delivery time by 25% and load times by 30%. Built scalable, accessible interfaces and real-time geospatial tools with React, Leaflet, and MapLibre for 10K+ daily users.",
      skills: [
        "React",
        "Redux",
        "Typescript",
        "Next.js",
        "TailwindCSS",
        "Leaflet",
        "Geojson",
        "Cypress",
        "CSS",
        "Websockt",
      ],
    },
    {
      timeline: {
        startYear: "2019",
        endYear: "2021",
      },
      companyName: "Apple",
      title: "Front End Engineer",
      description:
        "Developed a React + Redux app that cut manual workflows by 40% and improved data accuracy. Built a scalable component library used across teams, reducing dev time by 25%. Optimized builds and delivered performant, accessible UIs meeting Apple’s design and engineering standards.",
      skills: [
        "React",
        "Redux",
        "Javascript",
        "Cypress",
        "CSS",
        "Websockt",
        "HTML5",
        "Storybook",
      ],
    },
    {
      timeline: {
        startYear: "2017",
        endYear: "2019",
      },
      companyName: "Capillarity",
      title: "Front End Engineer Intern",
      description:
        "Developed the company’s primary marketing website with React, JavaScript, and CSS3, driving improved customer engagement. Reduced development costs by standardizing UI and creating reusable components for the design system.",
      skills: ["React", "Redux", "Javascript", "CSS", "HTML5"],
    },
  ],
  skills: [
    {
      heading: "User Interface",
      description:
        "Known as having hawk eyes by my peers, I strive for pixel-perfect finalized designs.",
      logo: ["Figma"],
    },
    {
      heading: "Component Libray",
      description:
        "Built and maintained a scalable React component library adopted across multiple teams, ensuring design consistency and accelerating development",
      logo: ["Figma", "React", "Tailwind"],
    },
    {
      heading: "Ecosystem",
      description:
        "Version control, code reviews, and CI/CD integration, Enforcing clean code and consistent formatting ",
      logo: ["Github", "NPM"],
    },
    {
      heading: "Testing",
      description:
        "Developed and maintained end-to-end testing frameworks using Cypress, ensuring UI reliability, regression coverage, and faster release cycles.",
      logo: ["Cypress", "jest"],
    },
    {
      heading: "Web",
      description:
        "My web projects are responsive, accessible, and intuitive for all walks of life while reassuring they will respond the same on any browser. ",
      logo: ["windows", "apple"],
    },
  ],
  email: "slokshah92@gmail.com",
  socialMedia: [
    {
      name: "GitHub",
      url: "https://github.com/Slokzx",
    },
    {
      name: "Instagram",
      url: "https://www.instagram.com/slokzx/",
    },
    {
      name: "Twitter",
      url: "https://x.com/slokzx",
    },
    {
      name: "Linkedin",
      url: "https://www.linkedin.com/in/slokshah/",
    },
  ],
  footer:
    " Loosely designed in Figma and coded in Visual Studio Code by Slok Shah with React.js and Tailwind CSS, deployed with Vercel. All text is set in the Inter typeface. © Copyright 2025",
  contact: {
    title: "Send me a message!",
    subtitle:
      " Got a question or proposal, or just want to say hello? Go ahead.",
    messagePlaceholder:
      "Hi, I think we need a design system for our products at Company X. How soon can you hop on to discuss this?",
    successMessage: {
      success: "Message sent! I will contact you ",
      loading: "Sending...",
      error: "Something went wrong.",
    },
    buttonText: "Shoot !",
    formAPI: "https://formspree.io/f/xeorwqoa",
  },
};
